...
def calculate_loss(viewpoint_camera, pc, render_pkg, opt, iteration, mask=None):
    tb_dict = {
    ...
    if mask is not None:
        # rendered_alpha.shape:[1,800,800]
        # mask.shape:[640000,1]
        mask = mask.view(1, 800, 800)
        loss += 0.01 * F.binary_cross_entropy(rendered_alpha.clip(1e-3, 1.0 - 1e-3), (mask > 0.5).float())
    loss += loss0
