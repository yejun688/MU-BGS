...
class ShapeTester:
    ...
    def run(self):
        ...
        env_dict = self.network.color_network.render_env_map()
        grid = [
            env_dict["env1"].permute(2, 0, 1),
        ]
        grid = make_grid(grid, nrow=1, padding=10)
        save_image(grid, os.path.join(self.base_save_path, "env1.png"))
        ...
        for _, ind in tqdm(enumerate(self.test_ids)):
            ...
            per_res_msg += f'{ind} psnr: {cur_psnr}, ssim: {cur_ssim}'
