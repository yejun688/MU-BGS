...
from network.entropy import specular_entropy
...
class TensoSDF(nn.Module):
    ...
    def forward(self, xyz_sampled, level_vol=None):
        # xyz_sampled : (rn*sn, 3)
        # plane + line basis
    ...
    def sdf_for_gs(self, xyz_sampled, level_vol=None, chunk=32768):
        # --- contraction ---
        xyz_sampled = contraction(xyz_sampled, self.aabb).reshape(-1, 3)

        # level volume
        level = (torch.zeros([xyz_sampled.shape[0], 1], device=xyz_sampled.device) 
                if level_vol is None else level_vol).view(-1, 1).unsqueeze(0).contiguous()

        # --- plane + line coordinates ---
        ...
        for idx in range(self.nplane):
            plane_coef_point.append(
                dr.texture(
                    planes[idx].permute(0, 2, 3, 1).contiguous(),
                    coordinate_plane[[idx]],
                    mip_level_bias=level,
                    boundary_mode="clamp",
                    max_mip_level=self.n_levels - 1
                ).permute(0, 3, 1, 2).contiguous().view(-1, xyz_sampled.shape[0])
            )

            line_coef_point.append(
                dr.texture(
                    lines[idx].permute(0, 2, 3, 1).contiguous(),
                    coordinate_line[[idx]],
                    mip_level_bias=level,
                    boundary_mode="clamp",
                    max_mip_level=self.n_levels - 1
        ...
        sigma_feature = plane_coef_point * line_coef_point  # (C, N)

        # --- embedding ---
        inputs_xyz = self.embed_fn(xyz_sampled) if self.embed_fn else xyz_sampled
        inputs_feat = self.sdf_feat_embed_fn(sigma_feature.T) if self.sdf_feat_embed_fn else sigma_feature.T

        # --- 分批送入 sdf_mat ---
        outputs = []
        N = inputs_xyz.shape[0]

        for start in range(0, N, chunk):
            end = start + chunk
            batch_in = torch.cat([inputs_feat[start:end], inputs_xyz[start:end]], dim=-1)
            batch_out = self.sdf_mat(batch_in)
            outputs.append(batch_out)

        return torch.cat(outputs, dim=0)[..., :1]
...
class ShapeShadingNetwork(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        FG_LUT = torch.from_numpy(np.fromfile('/data/6018_YEJUN/my-TensoSDF/assets/bsdf_256_256.bin', dtype=np.float32).reshape(1, 256, 256, 2))
        ...
        if self.cfg['sphere_direction']:
            self.outer_light = make_predictor_3layer(72*2, 3 + self.L4entropy * self.C4entropy, activation='exp', exp_max=exp_max)
        else:
            self.outer_light = make_predictor_3layer(72, 3 + self.L4entropy * self.C4entropy, activation='exp', exp_max=exp_max)
    ...
    def predict_specular_lights(self, points, feature_vectors, reflective, roughness, human_poses, step):
        ...
        return light, occ_prob, indirect_light, direct_light, human_light * human_weight, high_dim_light
    ...
    def predict_diffuse_lights(self, points, feature_vectors, normals):
        ...
        output_ =self.outer_light(ref)
        light = output_[..., :3]
        high_dim_light = output_[..., 3:]
        return light, high_dim_light

    def forward(self, points, normals, view_dirs, feature_vectors, human_poses=None, inter_results=False, step=None):
        ...
        diffuse_light, diffuse_high_dim_light = self.predict_diffuse_lights(points, feature_vectors, normals)
        diffuse_color = diffuse_albedo * diffuse_light
        
        # specular light
        specular_albedo = 0.04 * (1 - metallic) + metallic * albedo
        specular_light, occ_prob, indirect_light, direct_light, human_light, specular_high_dim_light = self.predict_specular_lights(points, feature_vectors, reflective, roughness, human_poses, step)

        entropy, min_alpha, min_specular = specular_entropy(
            incoming_spectrum=diffuse_high_dim_light.view(-1,self.L4entropy,self.C4entropy),   # 近似当作入射光
            exitant_spectrum=specular_high_dim_light.view(-1,self.L4entropy,self.C4entropy),   # 出射镜面光
            sigma_probability=0.1,
            alpha_resolution=10,
            specular_resolution=10
        )
        ...
        if inter_results:
            ...
            if self.cfg['has_radiance_field'] and step > self.cfg['radiance_field_step']:
                return color, radiance, occ_info, entropy
            else:
                return color, None, occ_info, entropy
